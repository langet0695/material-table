...
function buildConstructor(classRef, constructorBody, node) {
  const func = _core.types.functionDeclaration(
...
function transformClass(
  ...
  function pushBody() {
    ...
    for (const path of classBodyPaths) {
      ...
      if (_core.types.isClassMethod(node)) {
        ...
        const superReturns = [];
        path.traverse(
          _traverse.visitors.environmentVisitor({
  ...
  function pushDescriptors() {
    ...
    if (props.instance || props.static) {
      let args = [
        ...
        props.instance
          ? _core.types.arrayExpression(props.instance)
          : _core.types.nullLiteral(),
        props.static
          ? _core.types.arrayExpression(props.static)
          : _core.types.nullLiteral(),
      ];
  ...
  function wrapSuperCall(bareSuper, superRef, thisRef, body) {
    ...
    if (assumptions.superIsCallableConstructor) {
      ...
      if (
        ...
        _core.types.isIdentifier(bareSuperNode.arguments[1].argument, {
          name: "arguments",
        })
      ) {
      ...
      if (
        (_bareSuperNode$argume = bareSuperNode.arguments) != null &&
        _bareSuperNode$argume.length
      ) {
        const bareSuperNodeArguments = bareSuperNode.arguments;
        if (
          bareSuperNodeArguments.length === 1 &&
          _core.types.isSpreadElement(bareSuperNodeArguments[0]) &&
          _core.types.isIdentifier(bareSuperNodeArguments[0].argument, {
            name: "arguments",
          })
        ) {
      ...
      call = _core.types.callExpression(
        (0, _inlineCallSuperHelpers.default)(classState.file),
        args
      );
    }
    if (
  ...
  function verifyConstructor() {
    ...
    for (const thisPath of classState.superThises) {
      ...
      let exprPath = thisPath.parentPath.isSequenceExpression()
        ? thisPath.parentPath
        : thisPath;
      if (
    ...
    if (!bodyPaths.length || !bodyPaths.pop().isReturnStatement()) {
      body.pushContainer(
        ...
        _core.types.returnStatement(
          guaranteedSuperBeforeFinish ? thisRef() : buildAssertThisInitialized()
        )
      );
    }
  ...
  function pushMethod(node, wrapped) {
    ...
    const key =
      _core.types.isNumericLiteral(node.key) ||
      _core.types.isBigIntLiteral(node.key)
        ? _core.types.stringLiteral(String(node.key.value))
        : _core.types.toComputedKey(node);
  ...
  function processMethod(node) {
    if (assumptions.setClassMethods && !node.decorators) {
      ...
      const methodName = _core.types.memberExpression(
        _core.types.cloneNode(classRef),
        node.key,
        node.computed || _core.types.isLiteral(node.key)
      );
      const func = _core.types.functionExpression(
        node.id,
        node.params,
        node.body,
        node.generator,
      ...
      _core.types.inherits(func, node);
      const expr = _core.types.expressionStatement(
  ...
  function insertProtoAliasOnce() {
    if (classState.protoAlias === null) {
      ...
      const classProto = _core.types.memberExpression(
        classState.classRef,
  ...
  function setupClosureParamsArgs() {
    ...
    if (classState.isDerived) {
      ...
      if (classState.extendsNative) {
        arg = _core.types.callExpression(
  ...
  function classTransformer(path, file, builtinClasses, isLoose) {
    ...
    setState({
      extendsNative:
        _core.types.isIdentifier(classState.superName) &&
    ...
    if (!classState.pushedCreateClass) {
      body.push(
        _core.types.returnStatement(
          classState.isLoose
            ? _core.types.cloneNode(classState.classRef)
            : createClassHelper([_core.types.cloneNode(classState.classRef)])
        )
      );
    }
    body.unshift(classState.construct);
    const container = _core.types.arrowFunctionExpression(
      closureParams,
