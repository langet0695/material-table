...
function _helperMemberExpressionToFunctions() {
  const data = _interopRequireDefault(
    require("@babel/helper-member-expression-to-functions")
  );
...
function _helperOptimiseCallExpression() {
  const data = _interopRequireDefault(
    require("@babel/helper-optimise-call-expression")
  );
...
var _default = (0, _helperPluginUtils().declare)((api, options) => {
  ...
  const privateNameHandlerSpec = {
    ...
    get(member) {
      ...
      return _core().types.callExpression(
        ...
        [this.receiver(member), _core().types.cloneNode(map)]
      );
    },
    ...
    set(member, value) {
      ...
      return _core().types.callExpression(
        ...
        [this.receiver(member), _core().types.cloneNode(map), value]
      );
    },
  ...
  const privateNameHandlerLoose = {
    handle(member) {
      ...
      const { object } = member.node;
      member.replaceWith(
        _core().template.expression`BASE(REF, PROP)[PROP]`({
  ...
  const staticPrivatePropertyHandlerSpec = Object.assign(
    {},
      ...
      get(member) {
        ...
        return _core().types.callExpression(
          ...
          [
            ...
            _core().types.cloneNode(privateId),
          ]
        );
      },
  ...
  function buildClassPrivatePropertySpec(ref, path, initNodes, state) {
    ...
    initNodes.push(
      _core().template.statement`var MAP = new WeakMap();`({
        MAP: map,
      })
    );
    return () =>
          ...
          if (!computedPath.get("key").isConstantExpression()) {
            ...
            computedNodes.push(
              _core().types.variableDeclaration("var", [
                _core().types.variableDeclarator(ident, computedNode.key),
              ])
            );
        ...
        for (const prop of props) {
          if (prop.node.static) {
            if (prop.isPrivate()) {
              ...
              staticNodes.push(
                buildClassProperty(_core().types.cloneNode(ref), prop, state)
              );
            }
            ...
            instanceBody.push(
              buildClassProperty(_core().types.thisExpression(), prop, state)
            );
          }
        }
