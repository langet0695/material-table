...
function isPureVoid(node) {
  return (
    ...
    node.operator === "void" &&
    core.types.isPureish(node.argument)
  );
}
function unshiftForXStatementBody(statementPath, newStatements) {
  ...
  const hasShadowedBlockScopedBindings = Object.keys(
    bodyScopeBindings
  ).some((name) => scope.hasBinding(name));
...
const arrayUnpackVisitor = (node, ancestors, state) => {
  ...
  if (
    core.types.isIdentifier(node) &&
    core.types.isReferenced(node, ancestors[ancestors.length - 1].node) &&
...
class DestructuringTransformer {
  ...
  getExtendsHelper() {
    return this.useBuiltIns
      ? core.types.memberExpression(
          core.types.identifier("Object"),
          core.types.identifier("assign")
        )
  ...
  buildVariableAssignment(id, init) {
    ...
    if (
      core.types.isMemberExpression(id) ||
      core.types.isOptionalMemberExpression(id)
    )
      op = "=";
    let node;
    if (op) {
      node = core.types.expressionStatement(
        core.types.assignmentExpression(
          op,
          id,
          core.types.cloneNode(init) || this.scope.buildUndefinedNode()
        )
      );
    } else {
      let nodeInit;
  ...
  buildVariableDeclaration(id, init) {
    const declar = core.types.variableDeclaration("var", [
      core.types.variableDeclarator(
        core.types.cloneNode(id),
        core.types.cloneNode(init)
      ),
    ]);
  ...
  toArray(node, count) {
    if (
      ...
      (core.types.isIdentifier(node) && this.arrayRefSet.has(node.name))
    ) {
      return node;
    } else {
      ...
      return core.types.callExpression(
        scope.path.hub.addHelper(helperName),
        args
      );
    }
  }
  pushAssignmentPattern({ left, right }, valueRef) {
    ...
    const tempConditional = core.types.conditionalExpression(
      core.types.binaryExpression(
        "===",
        core.types.cloneNode(tempId),
  ...
  pushObjectProperty(prop, propRef) {
    ...
    const objRef = core.types.memberExpression(
      core.types.cloneNode(propRef),
      prop.key,
      prop.computed
    );
  ...
  pushUnpackedArrayPattern(pattern, arr) {
    const holeToUndefined = (el) =>
      el != null ? el : this.scope.buildUndefinedNode();
    for (let i = 0; i < pattern.elements.length; i++) {
      const elem = pattern.elements[i];
      if (core.types.isRestElement(elem)) {
        this.push(
          elem.argument,
          core.types.arrayExpression(arr.elements.slice(i).map(holeToUndefined))
        );
      } else {
  ...
  pushArrayPattern(pattern, arrayRef) {
    ...
    for (let i = 0; i < pattern.elements.length; i++) {
      ...
      if (core.types.isRestElement(elem)) {
        ...
        elemRef = core.types.callExpression(
          core.types.memberExpression(elemRef, core.types.identifier("slice")),
          [core.types.numericLiteral(i)]
        );
        this.push(elem.argument, elemRef);
      } else {
        elemRef = core.types.memberExpression(
          arrayRef,
          core.types.numericLiteral(i),
          true
        );
  ...
  init(pattern, ref) {
    if (
      !core.types.isArrayExpression(ref) &&
      !core.types.isMemberExpression(ref)
    ) {
      const memo = this.scope.maybeGenerateMemoised(ref, true);
      if (memo) {
        this.nodes.push(
          this.buildVariableDeclaration(memo, core.types.cloneNode(ref))
        );
        ref = memo;
      }
    }
...
function buildObjectExcludingKeys(
  excludedKeys,
  objRef,
  scope,
  addHelper,
  ...
  if (keys.length === 0) {
    ...
    if (!allLiteral) {
      keyExpression = core.types.callExpression(
        core.types.memberExpression(
          keyExpression,
          core.types.identifier("map")
        ),
    ...
    value = core.types.callExpression(
      addHelper(`objectWithoutProperties${objectRestNoSymbols ? "Loose" : ""}`),
      [core.types.cloneNode(objRef), keyExpression]
    );
  }
  return value;
}
function convertVariableDeclaration(
  ...
  if (
    nodesOut.length === 2 &&
    core.types.isVariableDeclaration(nodesOut[0]) &&
    core.types.isExpressionStatement(nodesOut[1]) &&
    core.types.isCallExpression(nodesOut[1].expression) &&
    ...
    if (
      core.types.isForStatement(path.parent, {
        init: node,
      }) &&
...
function convertAssignmentExpression(
  ...
  if (
    ...
    nodes.push(
      core.types.variableDeclaration("var", [
  ...
  const arrayLikeIsIterable =
    (_ref2 =
      (_options$allowArrayLi = options.allowArrayLike) != null
        ? _options$allowArrayLi
        : api.assumption("arrayLikeIsIterable")) != null
      ? _ref2
      : false;
      ...
      ForXStatement(path) {
        ...
        if (core.types.isPattern(left)) {
          ...
          if (statementBody.length === 0 && path.isCompletionRecord()) {
            nodes.unshift(
              core.types.expressionStatement(scope.buildUndefinedNode())
            );
          }
          nodes.unshift(
            core.types.expressionStatement(
