        ...
        _annexB_3_.annexB33FunctionsVisitor,
        {
          Loop(path, state) {
            ...
            for (const binding of bindings) {
              const { capturedInClosure } = (0, _loop.getUsageInBody)(
                binding,
                path
              );
            ...
            if (headPath && isBlockScoped(headPath.node)) {
              ...
              for (let name of names) {
                var _bodyScope;
                if (
                  (_bodyScope = bodyScope) != null &&
                  _bodyScope.hasOwnBinding(name)
                )
                  continue;
                ...
                const { usages, capturedInClosure, hasConstantViolations } = (0,
                _loop.getUsageInBody)(binding, path);
                if (
            ...
            if (needsBodyWrap) {
              const varPath = (0, _loop.wrapLoopBody)(
                path,
                captured,
                updatedBindingsUsages
              );
          ...
          ClassDeclaration(path) {
            ...
            if (
              !(0, _annexB_3_.isVarScope)(scope) &&
              scope.parent.hasBinding(id.name, {
                noUids: true,
              })
            ) {
...
function transformBlockScopedVariable(path, state, tdzEnabled) {
  ...
  const dynamicTDZNames = (0, _validation.validateUsage)(
    path,
    state,
    tdzEnabled
  );
  ...
  if (
    (isInLoop(path) && !(0, _loop.isVarInLoopHead)(path)) ||
    dynamicTDZNames.length > 0
  ) {
    for (const decl of path.node.declarations) {
      var _decl$init;
      (_decl$init = decl.init) != null
        ? _decl$init
