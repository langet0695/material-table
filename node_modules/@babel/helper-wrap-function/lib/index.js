...
function classOrObjectMethod(path, callId, ignoreFunctionLength) {
  ...
  if (shoudlForwardParams) {
    ...
    path
      .get("body.body.0.argument.callee.object.arguments.0")
  ...
  if (path.isArrowFunctionExpression()) {
    {
      ...
      path =
        (_path$arrowFunctionTo = path.arrowFunctionToExpression({
          noNewArrows,
        })) != null
          ? _path$arrowFunctionTo
          : path;
    }
    node = path.node;
  } else {
...
function wrapFunction(
  ...
  if (path.isMethod()) {
    ...
    const hadName = "id" in path.node && !!path.node.id;
    {
      ...
      (_path$ensureFunctionN = (_path = path).ensureFunctionName) != null
        ? _path$ensureFunctionN
        : (_path.ensureFunctionName = require("@babel/traverse").NodePath.prototype.ensureFunctionName);
    }
