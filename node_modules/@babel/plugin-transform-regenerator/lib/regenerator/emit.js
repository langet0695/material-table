...
class Emitter {
  ...
  assign(lhs, rhs) {
    return _core.types.expressionStatement(
      _core.types.assignmentExpression("=", _core.types.cloneNode(lhs), rhs)
    );
  }
  contextProperty(name, computed) {
    return _core.types.memberExpression(
      this.getContextId(),
      computed ? _core.types.stringLiteral(name) : _core.types.identifier(name),
      !!computed
    );
  }
  clearPendingException(tryLoc, assignee) {
    const catchCall = _core.types.callExpression(
      this.contextProperty("catch", true),
      [_core.types.cloneNode(tryLoc)]
    );
    if (assignee) {
  ...
  getContextFunction(id) {
    return _core.types.functionExpression(
      id || null,
      [this.getContextId()],
      _core.types.blockStatement([this.getDispatchLoop()]),
      false,
      false
    );
  }
  getDispatchLoop() {
    ...
    self.listing.forEach(function (stmt, i) {
      if (self.marked.hasOwnProperty(i)) {
        cases.push(
          _core.types.switchCase(_core.types.numericLiteral(i), (current = []))
        );
        alreadyEnded = false;
      }
      if (!alreadyEnded) {
    ...
    cases.push(
      _core.types.switchCase(this.finalLoc, []),
      _core.types.switchCase(_core.types.stringLiteral("end"), [
        _core.types.returnStatement(
          _core.types.callExpression(this.contextProperty("stop"), [])
        ),
      ])
    );
    return _core.types.whileStatement(
  ...
  getTryLocsList() {
    ...
    return _core.types.arrayExpression(
      this.tryEntries.map(function (tryEntry) {
        ...
        return _core.types.arrayExpression(
          locs.map((loc) => loc && _core.types.cloneNode(loc))
        );
      })
    );
  }
  ...
  explodeStatement(path, labelId = null) {
    ...
    switch (path.type) {
      ...
      case "ForInStatement":
        ...
        self.emitAssign(
          keyIterNextFn,
          _core.types.callExpression(util.runtimeProperty("keys"), [
          ...
          _core.types.memberExpression(
            _core.types.assignmentExpression(
              ...
              _core.types.callExpression(
                _core.types.cloneNode(keyIterNextFn),
                []
              )
            ),
            _core.types.identifier("done"),
            false
          ),
          after
        );
        self.emitAssign(
          path.node.left,
          _core.types.memberExpression(
            _core.types.cloneNode(keyInfoTmpVar),
      ...
      case "SwitchStatement":
        ...
        for (let i = cases.length - 1; i >= 0; --i) {
          const c = cases[i];
          if (c.test) {
            condition = _core.types.conditionalExpression(
              _core.types.binaryExpression(
                "===",
                _core.types.cloneNode(disc),
                c.test
              ),
              (caseLocs[i] = this.loc()),
              condition
      ...
      case "TryStatement":
        ...
        self.leapManager.withEntry(tryEntry, function () {
          ...
          if (finallyLoc) {
            ...
            self.emit(
              _core.types.returnStatement(
                _core.types.callExpression(self.contextProperty("finish"), [
  ...
  emitAbruptCompletion(record) {
    ...
    if (record.type === "break" || record.type === "continue") {
      abruptArgs[1] = this.insertedLocs.has(record.target)
        ? record.target
        : _core.types.cloneNode(record.target);
    ...
    this.emit(
      _core.types.returnStatement(
        _core.types.callExpression(this.contextProperty("abrupt"), abruptArgs)
      )
    );
  }
  ...
  explodeViaTempVar(tempVar, childPath, hasLeapingChildren, ignoreChildResult) {
    ...
    if (ignoreChildResult) {
      ...
      tempVar ||
      (hasLeapingChildren && !_core.types.isLiteral(result))
    ) {
    ...
    switch (path.type) {
      ...
      case "CallExpression":
        ...
        if (_core.types.isMemberExpression(calleePath.node)) {
          if (hasLeapingArgs) {
            ...
            newCallee = _core.types.memberExpression(
              _core.types.memberExpression(
                _core.types.cloneNode(newObject),
                newProperty,
          ...
          if (_core.types.isMemberExpression(newCallee)) {
            newCallee = _core.types.sequenceExpression([
              _core.types.numericLiteral(0),
              _core.types.cloneNode(newCallee),
            ]);
          }
        }
        if (hasLeapingArgs) {
          ...
          _core.types.arrayExpression(
            path.get("elements").map(function (elemPath) {
              ...
              if (elemPath.isSpreadElement()) {
                return _core.types.spreadElement(
                  self.explodeViaTempVar(
                    null,
      ...
      case "YieldExpression":
        ...
        if (arg && path.node.delegate) {
          ...
          const ret = _core.types.returnStatement(
            _core.types.callExpression(self.contextProperty("delegateYield"), [
              arg,
              _core.types.stringLiteral(result.property.name),
              after,
            ])
          );
          ret.loc = expr.loc;
        ...
        const ret = _core.types.returnStatement(
          _core.types.cloneNode(arg) || null
        );
        ret.loc = expr.loc;
        self.emit(ret);
        self.mark(after);
