...
}
{
  ...
  tokenTypes[5].updateContext = tokenTypes[7].updateContext = tokenTypes[23].updateContext = (
    context
  ) => {
    ...
    flowParseDeclareExportDeclaration(node, insideModule) {
      ...
      if (this.eat(65)) {
        ...
        if (
          this.match(75) ||
          this.isLet() ||
          ((this.isContextual(130) || this.isContextual(129)) && !insideModule)
        ) {
...
class CommentsParser extends BaseParser {
  ...
  finalizeComment(commentWS) {
    ...
    if (commentWS.leadingNode !== null || commentWS.trailingNode !== null) {
      ...
      if (
        this.input.charCodeAt(this.offsetToSourcePos(commentStart) - 1) === 44
      ) {
        switch (node.type) {
...
class LValParser extends NodeUtils {
  ...
  checkLVal(
    ...
    const isOptionalMemberExpression = this.isOptionalMemberExpression(
      expression
    );
...
var typescript = (superClass) =>
  class TypeScriptParserMixin extends superClass {
    ...
    tsInDisallowConditionalTypesContext(cb) {
      const oldInDisallowConditionalTypesContext = this.state
          ...
          this.state.startLoc,
          {
            propertyName:
              key.type === "Identifier" && !node.computed
                ? key.name
                : `[${this.input.slice(
                    this.offsetToSourcePos(key.start),
                    this.offsetToSourcePos(key.end)
                  )}]`,
          }
        );
      }
    ...
    reportReservedArrowTypeParam(node) {
      ...
      if (
        ...
        !((_node$extra2 = node.extra) != null && _node$extra2.trailingComma) &&
...
function validatePlugins(pluginsMap) {
  if (pluginsMap.has("decorators")) {
    ...
    const decoratorsBeforeExport = pluginsMap.get("decorators")
    ...
    const allowCallParenthesized = pluginsMap.get("decorators")
        ...
        if (
          ...
          throw new Error(
            `Plugin conflict between \`["pipelineOperator", { proposal: "hack", topicToken: "#" }]\` and \`${JSON.stringify(
              ["recordAndTuple", pluginsMap.get("recordAndTuple")]
            )}\`.`
          );
        }
      }
  ...
  if (pluginsMap.has("moduleAttributes")) {
    {
      ...
      const moduleAttributesVersionPluginOption = pluginsMap.get(
        "moduleAttributes"
      ).version;
...
class StatementParser extends ExpressionParser {
  ...
  parseExportDefaultExpression() {
    ...
    if (this.match(26)) {
      ...
      return this.parseClass(
        this.maybeTakeDecorators(this.parseDecorators(false), this.startNode()),
        true,
        true
      );
    }
  ...
  isPotentialImportPhase(isExport) {
    ...
    return (
      this.isContextual(105) || this.isContextual(97) || this.isContextual(127)
    );
  }
  applyImportPhase(node, isExport, phase, loc) {
    if (isExport) {
