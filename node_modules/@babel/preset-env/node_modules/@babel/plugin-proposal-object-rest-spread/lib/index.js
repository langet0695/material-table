...
const { isAssignmentPattern, isObjectProperty } = core.types;
{
  ...
  const property = core.types.objectProperty(
...
var index = helperPluginUtils.declare((api, opts) => {
  ...
  const supportsObjectAssign = !helperCompilationTargets.isRequired(
    "es6.object.assign",
    targets,
    {
  ...
  const objectRestNoSymbols =
    (_api$assumption2 = api.assumption("objectRestNoSymbols")) != null
      ...
      : loose;
  const pureGetters =
    (_api$assumption3 = api.assumption("pureGetters")) != null
      ? _api$assumption3
      : loose;
  const setSpreadProperties =
    (_api$assumption4 = api.assumption("setSpreadProperties")) != null
      ? _api$assumption4
      : loose;
  ...
  function replaceImpureComputedKeys(properties, scope) {
    ...
    for (const propPath of properties) {
      ...
      if (propPath.node.computed && !key.isPure()) {
        ...
        const declarator = core.types.variableDeclarator(
  ...
  function createObjectRest(path, file, objRef) {
        ...
        core.types.callExpression(getExtendsHelper(file), [
          ...
          core.types.sequenceExpression([
            core.types.callExpression(
              file.addHelper("objectDestructuringEmpty"),
    ...
    if (!allLiteral) {
      keyExpression = core.types.callExpression(
        core.types.memberExpression(
          core.types.arrayExpression(keys),
    ...
    visitor: {
      Function(path) {
        ...
        if (!idInRest) {
          ...
          pluginTransformParameters.convertFunctionParams(
            path,
      ...
      VariableDeclarator(path, file) {
        ...
        visitRestElements(path.get("id"), (path) => {
          ...
          if (
            ...
            !core.types.isIdentifier(originalPath.node.init)
          ) {
            ...
            originalPath.insertBefore(
              core.types.variableDeclarator(initRef, originalPath.node.init)
            );
            originalPath.replaceWith(
              core.types.variableDeclarator(
                originalPath.node.id,
                core.types.cloneNode(initRef)
              )
            );
            return;
          }
          ...
          refPropertyPath.forEach((prop) => {
            const { node } = prop;
            ref = core.types.memberExpression(
              ref,
              core.types.cloneNode(node.key),
              node.computed || core.types.isLiteral(node.key)
            );
          });
          ...
          insertionPath = insertionPath.insertAfter(
            core.types.variableDeclarator(argument, callExpression)
          )[0];
      ...
      AssignmentExpression(path, file) {
        ...
        if (leftPath.isObjectPattern() && hasRestElement(leftPath)) {
          ...
          const [
            ...
            argument,
            callExpression,
          ] = createObjectRest(leftPath, file, core.types.identifier(refName));
          if (impureComputedPropertyDeclarators.length > 0) {
            nodes.push(
              core.types.variableDeclaration(
                "var",
                impureComputedPropertyDeclarators
              )
            );
          ...
          nodes.push(
            core.types.expressionStatement(
              core.types.assignmentExpression("=", argument, callExpression)
            )
          );
          nodes.push(
            core.types.expressionStatement(core.types.identifier(refName))
          );
      ...
      ForXStatement(path) {
        ...
        if (!core.types.isVariableDeclaration(left)) {
          ...
          body.body.unshift(
            core.types.variableDeclaration(node.left.kind, [
      ...
      ArrayPattern(path) {
        ...
        visitRestElements(path, (path) => {
          ...
          objectPatterns.push(
            core.types.variableDeclarator(objectPattern.node, uid)
          );
        ...
        if (objectPatterns.length > 0) {
          ...
          statementPath.insertAfter(
            core.types.variableDeclaration(kind, objectPatterns)
          );
        }
      },
