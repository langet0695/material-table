  ...
  Please remove the \`require('@babel/polyfill')\` call or use \`useBuiltIns: 'entry'\` instead.`);
    ...
    MemberExpression: {
      enter(path, state) {
        ...
        if (
          has(_builtInDefinitions.definitions.staticMethods, evaluatedPropType)
        ) {
          const staticMethods =
            _builtInDefinitions.definitions.staticMethods[evaluatedPropType];
    ...
    VariableDeclarator(path, state) {
      ...
      for (let prop of node.id.properties) {
        ...
        if (
          !node.computed &&
          t.isIdentifier(prop) &&
          has(_builtInDefinitions.definitions.instanceMethods, prop.name)
        ) {
          const builtIn =
            _builtInDefinitions.definitions.instanceMethods[prop.name];
          addUnsupported(path, state.opts.polyfills, builtIn, this.builtIns);
        }
      }
    },
